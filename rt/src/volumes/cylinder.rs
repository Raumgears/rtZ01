...
impl Cylinder {
    pub fn new(center: Point3, length: f64, radius: f64, orientation: Vec3, mat: Rc<dyn Material>) -> Self {
...
impl Hittable for Cylinder {
    fn hit(&self, ray: &Ray, t_min: f64, t_max: f64, rec: &mut HitRecord) -> bool {
        ...
        let a = ray.direction().length_squared() - (dot(ray.direction(), self.orientation)).powf(2.);   
        let half_b = dot(ray.direction(), x) - (dot(ray.direction(), self.orientation) * dot(x, self.orientation));
        let c = x.length_squared() - dot(x, self.orientation).powf(2.) - self.radius * self.radius;
